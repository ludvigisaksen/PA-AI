...
export async function fetchTasksFromCatalyst() {
  if (!CATALYST_BASE_URL) {
    throw new Error('CATALYST_BASE_URL is not configured');
  }
  ...
  try {
    ...
    if (!resp.ok) {
      throw new Error(
        `[catalyst] GET /state/tasks failed (${resp.status}): ${buildResponseSnippet(respText)}`
      );
    }
    ...
    try {
      ...
      throw new Error(
        `[catalyst] Could not parse GET /state/tasks response JSON: ${buildResponseSnippet(respText)}`
      );
    }

    if (!data || !Array.isArray(data.tasks)) {
      throw new Error('[catalyst] GET /state/tasks response missing tasks array');
    }
    ...
    throw new Error(`[catalyst] Error calling GET /state/tasks: ${err.message}`);
  }
}